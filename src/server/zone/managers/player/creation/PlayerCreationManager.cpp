...
bool PlayerCreationManager::createCharacter(ClientCreateCharacterCallback* callback) const {
				...
				if (accountPermissionLevel < 9) {
					try {
						...
						if (res != nullptr && res->next()) {
							...
							if (timeVal.miliDifference() < 900000) {
								ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 15 minutes. Repeat attempts prior to 15 minutes elapsing will reset the timer.", 0x0);
								client->sendMessage(errMsg);
					...
					if (lastCreatedCharacter.containsKey(accID)) {
						...
						if (lastCreatedTime.miliDifference() < 900000) {
							ErrorMessage* errMsg = new ErrorMessage("Create Error", "You are only permitted to create one character per 15 minutes. Repeat attempts prior to 15 minutes elapsing will reset the timer.", 0x0);
							client->sendMessage(errMsg);
	...
	box->setPromptText("You are limited to creating one character per 15 minutes. Attempting to create another character or deleting your character before the 15 minutes timer expires will reset the timer.");
